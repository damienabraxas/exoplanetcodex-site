(function () {
  ...
  function forest(rows) {
    ...
    BANDS.forEach(function(band){
      ...
      here.forEach(function(p){var st=Number(p.sigma_stat||0),sy=Number(p.sigma_syst||0),sl=x(Number(p.A)-st),sw=x(Number(p.A)+st)-sl,yl=x(Number(p.A)-sy),yw=x(Number(p.A)+sy)-yl;
        out+='<div class="forest '+(p.tier==='GRADED'?'gradedrow':'')+'"><span class="forest-label"><b>'+esc(LABELS[p.instrument]||p.instrument)+'</b> · '+esc(p.display)+'<small>'+esc(p.holding)+' · n='+esc(p.n_lines)+'</small></span><span class="track"><i class="sysbar" style="left:'+yl+'%;width:'+Math.max(.4,yw)+'%"></i><i class="bar" style="left:'+sl+'%;width:'+Math.max(.4,sw)+'%"></i><i class="dot" style="left:'+x(Number(p.A))+'%"></i></span><span class="forest-value">'+num(p.A)+' <small>±'+num(st)+' stat ±'+num(sy)+' syst</small></span></div>';
      });
  ...
  Promise.all([fetchText(urls.product),fetchText(urls.holdings),fetchText(urls.instruments)]).then(function(parts){
    var feed=JSON.parse(parts[0]), holdings=csv(parts[1]), instruments=csv(parts[2]), telluric={}, catalog={};
    ...
    var top=INSTRUMENTS.map(function(i){return preferred(graded.filter(function(p){return p.instrument===i;}));}).filter(Boolean);
    ...
    root.innerHTML='<p class="product-feed-meta">Live source: <strong>codex.element_product/1 · '+esc(element)+'.json v'+esc(feed.version)+'</strong> · updated '+esc(feed.updated_at)+' · cache-busted on every load · '+excluded.length+' telluric-ineligible live rows refused</p>'+
      ...
      '<section class="product-section"><h2>Error-bar forest</h2><p class="product-section-intro">RYA-935 geometry: per-holding rows with statistical and systematic uncertainties kept separate.</p>'+forest(live.filter(function(p){return p.ion==='I';}))+'</section>'+
